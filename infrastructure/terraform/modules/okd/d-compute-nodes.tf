...
data "ct_config" "compute_customized_ignition" {
  ...
  content                 = templatefile("${path.module}/../../ansible/files/configs/vm_customized_ignition.yaml.tftpl", {
    merge_ignition_source = "http://${var.lb_vip}:8080/${var.okd_cluster_prefix}/worker.ign"
...
resource "proxmox_vm_qemu" "cloudinit-okd-compute" {
  depends_on  = [
    proxmox_vm_qemu.cloudinit-okd-primary,
  ]
  ...
  desc        = "Cloudinit OKD Compute"
  vmid        = tonumber(var.scos_template_id) + tonumber(var.compute_initial_ip_suffix) + count.index
